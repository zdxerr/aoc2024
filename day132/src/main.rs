...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (_, [xa, ya, xb, yb, x, y]) in expr.captures_iter(input.as_str()).map(|c| c.extract()) {
        let (xa, ya, xb, yb, x, y, xa_, ya_, xb_, yb_, x_, y_) = (
            ...
            xb.parse::<u64>().unwrap(),
            yb.parse::<u64>().unwrap(),
            x.parse::<u64>().unwrap() + 10_000_000_000_000,
            y.parse::<u64>().unwrap() + 10_000_000_000_000,
            xa.parse::<f64>().unwrap(),
            ya.parse::<f64>().unwrap(),
            xb.parse::<f64>().unwrap(),
            yb.parse::<f64>().unwrap(),
            x.parse::<f64>().unwrap() + 10_000_000_000_000.0,
            y.parse::<f64>().unwrap() + 10_000_000_000_000.0,
        );
